...
std::vector<unsigned int> find_max_profit(const std::vector<std::vector<unsigned int> >& profits, std::vector<unsigned int> maxed_compagnies);
//méthode qui retire les colonnes inutiles du tableau profit
std::vector<std::vector<unsigned int>> update_profits(const std::vector<std::vector<unsigned int> >& profits, int new_budget);
#endif
